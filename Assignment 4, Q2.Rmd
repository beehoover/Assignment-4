...
ggplot(lb_sz_sum_2, 
       ...
       geom_errorbar(aes(ymax=SIZE+se, ymin=SIZE-se), 
                     width=0.0, size=0.5, color="black")  +
       geom_text(aes(label=c("ab","b","b","b","a"),
                 hjust=offset.h, vjust=offset.v)) +              
       labs(x = "Sample location", 
            y = "Mean Size (mm)",
            caption = "Bar plot of means for each SITE of a one-way analysis of variance (ANOVA).  Error indicates standard error of the mean.  Bars sharing the same letter are not significantly different according to Tukey’s HSD test.")  +
